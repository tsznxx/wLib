...
using namespace std;
	...
	//Instead, use mathimf.h by including a seperate file
	#include "gamma.h"
...
void openct(structure *ct,const char *ctfile) {
	...
	in.open(ctfile);
	if(!in.is_open()){
	  cerr << "Error opening ct file " << ctfile << '\n';
	  exit(1);
	}
	in >> count;
	j = 0;
	...
	if (count == -100) { //this is a CCT formatted file:
		...
		//FOR NOW, DISABLE READING CT FILES WITH STACKING INFO: THIS SEEMS TO BE A PROBLEM WITH THE WEBSERVER
		//i = (int) strlen(temp);
		//if (i==35) {
		//	ct->stacking = true;
		//}
